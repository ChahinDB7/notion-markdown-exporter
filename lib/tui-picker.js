...
function defaultTextHint(defaultValue, allowBack, maxLength) {
	const parts = ['Enter submit'];
	if (defaultValue) parts.push(`empty = "${defaultValue}"`);
	if (maxLength) parts.push(`max ${maxLength} chars`);
	if (allowBack) parts.push('Esc back');
	parts.push('Ctrl-C cancel');
	return parts.join(' · ');
}
...
export async function promptText({
	...
	// If the seeded initialValue exceeds maxLength, hard-truncate so the
	// counter and the buffer never disagree.
	if (typeof maxLength === 'number' && maxLength > 0 && buffer.length > maxLength) {
		buffer = buffer.slice(0, maxLength);
	}
	let caret = buffer.length;
	let error = '';
	const limited = typeof maxLength === 'number' && maxLength > 0;
	const hintLine = defaultTextHint(defaultValue, allowBack, limited ? maxLength : undefined);

	const render = () => {
		let out = '\n' + message + '\n  ' + hintLine + '\n\n';
		const counter = limited ? `  (${buffer.length}/${maxLength})` : '';
		out += '  ❯ ' + renderInputLine(buffer, caret) + counter + '\n';
		if (error) out += '\n  ❌ ' + error + '\n';
		return out;
	};
	...
	return runPrompt({
		...
		onKey: (key, finish) => {
			...
			if (key === 'space') {
				// Block leading whitespace: no space at position 0.
				if (caret === 0) return;
				// Block 2+ consecutive spaces: refuse if the char on either side
				// of the caret is already a space.
				if (buffer[caret - 1] === ' ' || buffer[caret] === ' ') return;
				if (limited && buffer.length >= maxLength) return;
				buffer = buffer.slice(0, caret) + ' ' + buffer.slice(caret);
				caret++;
				error = '';
				return;
			}
